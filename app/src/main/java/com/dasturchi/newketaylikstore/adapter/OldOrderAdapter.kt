...
import com.dasturchi.newketaylikstore.databinding.ItemOldOrderBinding
import com.dasturchi.newketaylikstore.model.Order
import com.dasturchi.newketaylikstore.ui.main.order.OrderDetailActivity

class OldOrderAdapter : RecyclerView.Adapter<OldOrderAdapter.OrderHolder>() {
    private var orderList: List<Order> = ArrayList()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): OrderHolder {
        val inflater = LayoutInflater.from(parent.context)
        val binding = ItemOldOrderBinding.inflate(inflater,parent,false)
    ...
    inner class OrderHolder(val binding: ItemOldOrderBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: Order) {
            binding.item = item
